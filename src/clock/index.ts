...
export const toDate = (date?: string, format = "ZZ", masked = BR_DATE_MASK) => {
	if (date) {
		return moment(date, format).format(masked);
	}
	return moment().format(masked);
};

export const dateHour = (date?: string, format = "ZZ") => {
	if (date) {
		return moment(date, format).format(BR_DATETIME_MASK);
	}
